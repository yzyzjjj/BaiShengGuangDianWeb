function pageReady(){$(".sidebar-mini").addClass("sidebar-collapse");$(".ms2").select2({matcher});$("#deviceSelect,#productionDeviceSelect").select2({allowClear:!0,placeholder:"请选择",multiple:!0,width:"100%"});const n=new Promise(n=>getWarnSet(n,"#deviceWarn",1)),t=new Promise(n=>getClass(n)),i=new Promise(n=>getScript(n));Promise.all([n,t,i]).then(n=>{n[0].length?showAppointWarnSet(n[0][0]):(getDevice(null,{scriptId:n[2]},"#deviceSelect"),getScriptItem())});$("#deviceSelect,#productionDeviceSelect").on("select2:select",function(){const n=$(this).val();if(~n.indexOf("0")){const t=$(this).find("option"),n=[];for(let i=0,r=t.length;i<r;i++)n.push(t.eq(i).val());n.splice(n.indexOf("0"),1);$(this).val(n).trigger("change")}});$("#deviceWarn").on("select2:select",()=>getDeviceWarnSet());$("#scriptSelect").on("select2:select",function(){getDevice(null,{scriptId:$(this).val()},"#deviceSelect");getScriptItem()});$("#productionLi").one("click",function(){const n=new Promise(n=>getWarnSet(n,"#productionWarn",2)),t=new Promise(n=>getStep(n));Promise.all([n,t]).then(n=>{const i=n[0],t=n[1];$("#productionDeviceStep").html(setOptions(t,"StepName")).on("change",function(){const r=$(this).val()>>0,n=t.filter(n=>n.Id==r);if(n.length){const t=n[0];$("#productionDeviceType").attr("categoryId",t.DeviceCategoryId);$("#productionDeviceType").val(t.CategoryName);t.DeviceCategoryId?($("#productionDeviceSelectDiv").removeClass("hidden"),new Promise(n=>getDevice(n,{categoryId:t.DeviceCategoryId},"#productionDeviceSelect")).then(()=>{const t=$("#productionWarn").val()>>0,n=i.filter(n=>n.Id==t);if(n.length){const t=n[0];$("#productionDeviceSelect").val(t.DeviceList).trigger("change")}})):($("#productionDeviceSelect").empty(),$("#productionDeviceSelectDiv").addClass("hidden"))}});if(n[0].length)showProductionWarnSet(n[0][0]);else{$("#productionDeviceSelect").addClass("hidden");n[1].length&&n[1][0].DeviceCategoryId&&($("#productionDeviceSelect").removeClass("hidden"),getDevice(null,{categoryId:n[1].DeviceCategoryId},"#productionDeviceSelect"));const t=n[1];if(t.length){const n=t[0];$("#productionDeviceType").text(n.CategoryName);n.DeviceCategoryId?($("#productionDeviceSelectDiv").removeClass("hidden"),new Promise(n=>getDevice(n,{categoryId:rData.CategoryId},"#productionDeviceSelect")).then(()=>$("#productionDeviceSelect"))):($("#productionDeviceSelect").empty(),$("#productionDeviceSelectDiv").addClass("hidden"))}getProductionItem()}})});$("#productionDeviceType").on("select2:select",function(){getDevice(null,{categoryId:$(this).val()},"#productionDeviceSelect")});$("#productionWarn").on("select2:select",()=>getProductionWarnSet());$("#devBox .dTable").on("change",".interval",function(){var n=$(this).val();$(this).siblings().removeClass("hidden");n==1?$(this).siblings().addClass("hidden"):n==2&&$(this).siblings(".frequency").addClass("hidden")})}function getWarnSet(n,t,i){const r=$("#typeBox li.active").val();ajaxPost("/Relay/Post",{opType:1200,opData:JSON.stringify({first:!0,dataType:i,type:r})},i=>{if(i.errno!=0){layer.msg(i.errmsg);return}const r=i.datas;$(t).empty().append(setOptions(r,"Name"));n(r)})}function getDeviceWarnSet(){_trData[device]=[];const n=$("#deviceWarn").val();if(isStrEmptyOrUndefined(n)){layer.msg("请选择预警名");return}getAppointWarnSet(n,n=>showAppointWarnSet(n))}function getProductionWarnSet(){const n=$("#productionWarn").val();if(isStrEmptyOrUndefined(n)){layer.msg("请选择预警名");return}getAppointWarnSet(n,n=>showProductionWarnSet(n))}function getAppointWarnSet(n,t){const i=$("#typeBox li.active").val(),r=$("#navUl li.active").val();ajaxPost("/Relay/Post",{opType:1200,opData:JSON.stringify({qId:n,first:!0,dataType:r,type:i})},n=>{if(n.errno!=0){layer.msg(n.errmsg);return}t(n.datas[0])})}function showAppointWarnSet(n){setThreeTable(n.Items,!1);$("#warnName").val(n.Name);$("#isStart").iCheck(n.Enable?"check":"uncheck");$("#deviceType").val(n.ClassId).trigger("change");$("#scriptSelect").val(n.ScriptId).trigger("change");new Promise(t=>getDevice(t,{scriptId:n.ScriptId},"#deviceSelect")).then(()=>$("#deviceSelect").val(n.DeviceList).trigger("change"))}function showProductionWarnSet(n){setProduction(n.Items,!1);$("#productionWarnName").val(n.Name);$("#productionIsStart").iCheck(n.Enable?"check":"uncheck");$("#productionDeviceStep").val(n.StepId).trigger("change");$("#productionDeviceType").attr("categoryId",n.CategoryId);$("#productionDeviceType").val(n.CategoryName);n.CategoryId?($("#productionDeviceSelectDiv").removeClass("hidden"),new Promise(t=>getDevice(t,{categoryId:n.CategoryId},"#productionDeviceSelect")).then(()=>$("#productionDeviceSelect").val(n.DeviceList).trigger("change"))):($("#productionDeviceSelect").empty(),$("#productionDeviceSelectDiv").addClass("hidden"))}function deviceRefreshTable(){$("#deviceWarn").val()&&$("#deviceWarn :selected").text()==$("#warnName").val().trim()?getDeviceWarnSet():getScriptItem()}function productionRefreshTable(){$("#productionWarn").val()&&$("#productionWarn :selected").text()==$("#productionWarnName").val().trim()?getProductionWarnSet():getProductionItem()}function productionSumRefreshTable(){$("#productionIsCollect").trigger("ifChanged")}function getStep(n){ajaxPost("/Relay/Post",{opType:150},t=>{if(t.errno!=0){layer.msg(t.errmsg);return}const i=t.datas;n(i)},0)}function getClass(n){ajaxPost("/Relay/Post",{opType:168,opData:JSON.stringify({menu:!0})},t=>{const i=t.datas;if(t.errno!=0){layer.msg(t.errmsg);return}$("#deviceType").empty().append(setOptions(t.datas,"Class"));n(i)},0)}function getScript(n){ajaxPost("/Relay/Post",{opType:113,opData:JSON.stringify({menu:!0})},t=>{if(t.errno!=0){layer.msg(t.errmsg);return}$("#scriptSelect").empty().append(setOptions(t.datas,"ScriptName"));n(t.datas[0].Id)},0)}function getDevice(n,t,i){ajaxPost("/Relay/Post",{opType:100,opData:JSON.stringify(t)},t=>{if(t.errno!=0){layer.msg(t.errmsg);return}const r=t.datas;$(i).html(setOptions(r,"Code"));n&&n()},0)}function getWarnAllItem(n,t){ajaxPost("/Relay/Post",{opType:1204,opData:JSON.stringify(n)},n=>{if(n.errno!=0){layer.msg(n.errmsg);return}t(n.datas)},0)}function getScriptItem(){const n=$("#scriptSelect").val();if(isStrEmptyOrUndefined(n)){layer.msg("请选择脚本");return}getWarnAllItem({sId:n,isNew:!0,dataType:1},n=>setThreeTable(n,!0))}function setThreeTable(n){_choseData[device]=[];_trData[device]=[];_dataTable[device]=[];const i={1:[],2:[],3:[]},r={1:"npcVarList",2:"npcInsList",3:"npcOutList"};for(var t in i)i[t]=n.filter(n=>n.VariableTypeId==t),setDataTable(device,r[t],i[t],_trData[device])}function getProductionItem(){getWarnAllItem({isNew:!0,dataType:2},n=>setProduction(n,!0))}function setProduction(n){_choseData[production]=[];_trData[production]=[];_dataTable[production]=[];setDataTable(production,"productionList",n,_trData[production])}function setDataTable(n,t,i,r){let u="";n==device?u="DictionaryId":n==production&&(u="ItemType");const f=t;if(t=`#${t}`,_dataTable[n][f])updateTable(_dataTable[n][f],i);else{const h=n=>{const t=n.Frequency,i=n.Count;return`<span class="textOn"></span>
                    <div class="flexStyle hidden textIn" style="justify-content:center">
                        ...
                        <select class="form-control interval" style="min-width:70px">
                            ${["不设置","每次","连续","秒","分","小时","天","周","月","年"].reduce((n,t,i)=>n+(i!=0?`<option value="${i}">${t}</option>`:``),"")}
                        </select>
