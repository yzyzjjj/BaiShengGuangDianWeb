function pageReady(){$("#kanBanList").select2();$("#deviceSelect").select2({allowClear:!0,placeholder:"请选择",multiple:!0});new Promise(n=>getDevice(n)).then(n=>{$("#deviceSelect").on("select2:select",function(){var t=$(this).val();~t.indexOf("0")&&$(this).val(n).trigger("change")});return new Promise(n=>kanBanChange(n))}).then(()=>$("#navUl .kanBan:first").click());var n=0;$("#navUl").on("click",".kanBan",function(){$("#fullScreenBtn").removeClass("hidden");var t=$(this).attr("href"),i=t.replace("#kanBan","");setChart(t,i);clearInterval(n);n=setInterval(()=>setChart(t,i),5e3)});$("#fullScreenBtn").on("click",function(){$("#tabBox").toggleClass("panel-fullscreen");var n=$("#tabBox").hasClass("panel-fullscreen");$(this).toggleClass("glyphicon-fullscreen glyphicon-repeat").prop("title",n?"还原":"全屏放大");n?$("#firstNavLi").addClass("hidden"):$("#firstNavLi").removeClass("hidden");fullScreen(n)});$("#firstNavLi").on("click",function(){clearInterval(n);$("#fullScreenBtn").addClass("hidden")})}function fullScreenCarousel(){$("#fullScreenCarousel").empty().append(_carouselBox).animate({top:0,opacity:1},1e3,"linear",()=>fullScreen(!0));setFullScreenCarousel($("#carouselInner .item:first").attr("id"));$("#carousel-example-generic").carousel({interval:1e4}).on("slide.bs.carousel",n=>setFullScreenCarousel(n.relatedTarget.id)).on("mouseenter",()=>$("#carousel-example-generic .isShow").removeClass("hidden")).on("mouseleave",()=>$("#carousel-example-generic .isShow").addClass("hidden"))}function setFullScreenCarousel(n){var t=n.replace("carousel_kanBan","");setChart(`#${n}`,t)}function cancelFullScreenCarousel(){$("#carousel-example-generic").carousel("pause").off("slide.bs.carousel mouseenter mouseleave");$("#fullScreenCarousel").animate({top:"-100%",opacity:0},1e3,"linear",function(){$(this).empty();fullScreen(!1)})}function setChart(n,t){var i={opType:509,opData:JSON.stringify({type:t})};ajaxPost("/Relay/Post",i,t=>{var i,a,e,v,o,y,r,f,h,c,l,tt;if(t.errno!=0){layer.msg(t.errmsg);return}i=t.data;$(`${n}_time`).text(i.Time);a=setPieChart(`${n}_detail`,"Rate",i.AllProcessRate,"#0099ff");$(`${n}_yxTime`).text(codeTime(i.RunTime));$(`${n}_jgTime`).text(codeTime(i.ProcessTime));$(`${n}_xzTime`).text(codeTime(i.IdleTime));$(`${n}_lly`).text(a);e=`${i.MaxUse}台`;v=setPieChart(`${n}_max`,`Max
${e}`,i.MaxUseRate,"#00a65a");$(`${n}_maxTs`).text(`${i.MaxSimultaneousUseRate}台`);$(`${n}_maxTNum`).text(e);$(`${n}_maxSyl`).text(v);o=`${i.MinUse}台`;y=setPieChart(`${n}_min`,`Min
${o}`,i.MinUseRate,"#f39c12");$(`${n}_minTs`).text(`${i.MinSimultaneousUseRate}台`);$(`${n}_minTNum`).text(o);$(`${n}_minSyl`).text(y);var u=i.AllDevice,p=i.NormalDevice,s=i.ProcessDevice,w=i.IdleDevice,b=i.FaultDevice,k=i.ConnectErrorDevice;for(setPieChart(`${n}_state_all`,`${u}台`,u/u,"green","总设备"),setPieChart(`${n}_state_normal`,`${p}台`,p/u,"blue","正常运行"),setPieChart(`${n}_state_process`,`${s}台`,s/u,"#0099ff","加工中"),setPieChart(`${n}_state_idle`,`${w}台`,w/u,"#cc33ff","闲置中"),setPieChart(`${n}_state_fault`,`${b}台`,b/u,"red","故障中"),setPieChart(`${n}_state_error`,`${k}台`,k/u,"#ff00cc","连接异常"),h=i.UseCodeList||[],c="",r=0,f=h.length;r<f;r++)c+=`<div style="border:1px solid;padding:5px 10px;margin:3px;border-radius:20%;background:#bfa;font-weight:bold">${h[r]}</div>`;$(`${n}_dev`).empty().append(c||'<label style="color:red;font-size:18px">无<\/label>');var d=i.SingleProcessRate,g=[],nt=[];for(r=0,f=d.length;r<f;r++)l=d[r],g.push(l.Code),nt.push((l.Rate*100).toFixed(2));tt={tooltip:{trigger:"axis",formatter:"{a}<br/>{b} : {c}%",axisPointer:{type:"shadow"}},grid:{left:"3%",right:"3%",top:"6%",bottom:"8%",containLabel:!0},xAxis:{type:"category",axisLine:{onZero:!1},data:g},yAxis:{name:"百分比",type:"value",scale:!0,axisLabel:{show:!0,formatter:"{value}%"}},series:{name:"利用率",type:"bar",areaStyle:{normal:{}},barWidth:"60%",label:{show:!0,position:"top",formatter:"{c}%"},itemStyle:{color:n=>n.value<30?"red":"green"},data:nt},dataZoom:[{type:"inside"},{handleIcon:_handleIcon,handleSize:"80%",handleStyle:{color:"#fff",shadowBlur:3,shadowColor:"rgba(0, 0, 0, 0.6)",shadowOffsetX:2,shadowOffsetY:2}}]};echarts.init($(`${n}_bar`)[0]).setOption(tt,!0);$(n).off("resize").on("resize",function(){clearTimeout(this.time);this.time=setTimeout(()=>{for(var t=$(this).find(".chart"),n=0,i=t.length;n<i;n++)echarts.init(t[n]).resize()},200)});var it=i.ProductionList,rt="",ut="",ft="",et="",ot="";for(r=0,f=it.length;r<f;r++){const n=it[r];rt+=`<td class="text-blue">${n.Code}</td>`;ut+=`<td>${n.FaChu}</td>`;ft+=`<td>${n.HeGe}</td>`;et+=`<td>${n.LiePian}</td>`;ot+=`<td>${(n.Rate*100).toFixed(2)}%</td>`}$(`${n}_devs`).text(s);$(`${n}_pros`).text(i.FaChu);$(`${n}_heGes`).text(i.HeGe);$(`${n}_liePians`).text(i.LiePian);$(`${n}_rates`).text(`${(i.Rate*100).toFixed(2)}%`);$(`${n}_devOps`).find("td:not(:first)").remove().end().append(rt);$(`${n}_proOps`).find("td:not(:first)").remove().end().append(ut);$(`${n}_heGeOps`).find("td:not(:first)").remove().end().append(ft);$(`${n}_liePianOps`).find("td:not(:first)").remove().end().append(et);$(`${n}_rateOps`).find("td:not(:first)").remove().end().append(ot)},0)}function setPieChart(n,t,i,r,u){i=i||0;var e=`${(i*100).toFixed(2)}%`,f={color:["#cdcdcd",r],series:{type:"pie",center:["50%","50%"],radius:["65%","100%"],hoverAnimation:!1,animation:!1,silent:!0,label:{show:!0,position:"center",formatter:`${t}
${e}`,textStyle:{color:r,fontSize:18,fontWeight:700}},data:[1-i,i]}};return u&&(f.title={text:u,left:"center",textStyle:{color:r,fontSize:16}},f.series.center=["50%","57%"],f.series.radius=["57%","85%"]),echarts.init($(n)[0]).setOption(f,!0),e}function kanBanChange(n){new Promise(n=>getKanBanList(n)).then(t=>{$("#kanBanList").off("select2:select").on("select2:select",function(){var i=$(this).val(),n=t[i];$("#kanBanName").val(n.Name);$("#deviceSelect").val(n.DeviceIdList).trigger("change");$("#isShow").iCheck(n.IsShow?"check":"uncheck")});$("#kanBanList").trigger("select2:select");n&&n()})}function getKanBanList(n){var t={opType:509,opData:JSON.stringify({type:-1})};ajaxPost("/Relay/Post",t,t=>{var r,y,i,p,u;if(t.errno!=0){layer.msg(t.errmsg);return}var f=`<div class="box box-primary">
