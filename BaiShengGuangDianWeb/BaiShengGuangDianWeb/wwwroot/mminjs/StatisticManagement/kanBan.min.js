...
${e}`,textStyle:{color:r,fontSize:18,fontWeight:700}},data:[1-i,i]}};return u&&(f.title={text:u,left:"center",textStyle:{color:r,fontSize:16}},f.series.center=["50%","57%"],f.series.radius=["57%","85%"]),echarts.init($(n)[0]).setOption(f,!0),e}function kanBanChange(){new Promise(n=>getKanBanList(n)).then(n=>{$("#kanBanList").off("select2:select").on("select2:select",function(){var i=$(this).val(),t=n[i];$("#kanBanName").val(t.Name);$("#deviceSelect").val(t.DeviceIdList).trigger("change");$("#isShow").iCheck(t.IsShow?"check":"uncheck")});$("#kanBanList").trigger("select2:select")})}function getKanBanList(n){var t={opType:509,opData:JSON.stringify({type:-1})};ajaxPost("/Relay/Post",t,t=>{var r,p,i,w,u,f;if(t.errno!=0){layer.msg(t.errmsg);return}var e=`<div class="row">
                    <div class="col-md-4">
                ...
                <div class="row">
                ...
                </div>`,b=`<div class="tab-pane fade in" id="{0}">${e}</div>`,k=`<div class="item {2}" id="{0}">${e}<div class="carousel-caption text-primary"><h3 class="text-primary">{1}</h3></div></div>`,d=`<div class="box box-primary">
                        <div class="box-header with-border">
                    ...
                    </div>`,o=t.data,s="",h="",c="",l="",a="",v={},y=0;for(r=0,p=o.length;r<p;r++)i=o[r],v[i.Id]=i,w=`${i.Order}-${i.Name}`,s+=`<option value=${i.Id} order=${i.Order}>${w}</option>`,i.IsShow&&(u=`kanBan${i.Id}`,f=y?"":"active",h+=`<li><a href="#${u}" class="kanBan" data-toggle="tab" aria-expanded="false">${i.Name}</a></li>`,c+=b.format(u),l+=`<li data-target="#carousel-example-generic" data-slide-to=${y++} class=${f}></li>`,a+=k.format(`carousel_${u}`,i.Name,f));$("#kanBanList").empty().append(s);$("#firstNavLi").nextAll().remove().end().after(h);$("#setKanBan").nextAll().remove().end().after(c);_carouselBox=d.format(l,a);n(v)})}function fullScreen(n){if(n){var t=document.body;t.requestFullscreen?t.requestFullscreen():t.mozRequestFullScreen?t.mozRequestFullScreen():t.webkitRequestFullScreen?t.webkitRequestFullScreen():t.msRequestFullscreen&&t.msRequestFullscreen()}else document.mozCancelFullScreen?document.mozCancelFullScreen():document.webkitCancelFullScreen?document.webkitCancelFullScreen():document.msExitFullscreen?document.msExitFullscreen():document.exitFullscreen&&document.exitFullscreen()}function getDevice(n){ajaxPost("/Relay/Post",{opType:100},t=>{var i,s,r,u;if(t.errno!=0){layer.msg(t.errmsg);return}var f=t.datas,e='<option value="0">全部<\/option>',o=[];for(i=0,s=f.length;i<s;i++)r=f[i],u=r.Id,e+=`<option value=${u}>${r.DeviceName}</option>`,o.push(u);$("#deviceSelect").empty().append(e);n(o)})}function delKanBanList(){var n=$("#kanBanList").val(),t;if(isStrEmptyOrUndefined(n)){layer.msg("请选择需要删除的看板");return}t=()=>{var t={opType:512,opData:JSON.stringify({id:n})};ajaxPost("/Relay/Post",t,n=>{layer.msg(n.errmsg),n.errno==0&&kanBanChange()})};showConfirm(`删除：${$("#kanBanList :selected").text()}`,t)}function setAddKanBan(n){var o=n?++$("#kanBanList").children().length:$("#kanBanList :selected").attr("order"),u=$("#kanBanName").val().trim(),t,f,i,r,e;if(isStrEmptyOrUndefined(u)){layer.msg("看板名称不能为空");return}if(t=$("#deviceSelect").val(),isStrEmptyOrUndefined(t)){layer.msg("请选择设备");return}if(f=$("#isShow").is(":checked"),i={Name:u,DeviceIds:t.join(","),Order:o>>0,IsShow:f},!n){if(r=$("#kanBanList").val(),isStrEmptyOrUndefined(r)){layer.msg("请选择需要设置的看板");return}i.Id=r>>0}e={opType:n?510:511,opData:JSON.stringify(i)};ajaxPost("/Relay/Post",e,n=>{layer.msg(n.errmsg),n.errno==0&&kanBanChange()})}var _carouselBox;