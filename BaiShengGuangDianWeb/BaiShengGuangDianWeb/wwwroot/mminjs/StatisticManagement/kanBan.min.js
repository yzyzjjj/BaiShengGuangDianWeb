function pageReady(){var n,i,r;$(".sidebar-mini").addClass("sidebar-collapse");_permissionList[675]={uIds:[]};_permissionList[676]={uIds:["firstNavLi","setKanBan"]};_permissionList=checkPermissionUi(_permissionList);$("#kanBanList").select2();$("#deviceSelect").select2({allowClear:!0,placeholder:"请选择",multiple:!0});$("#navUl").on("click","a",function(){var n=$(this).attr("href").replace("#",""),f=n.replace("kanBan_","");const u=parseInt(f);if(curId!=u){curId=u;let r=-1;const i=kanBans[curId];i&&(r=i.Type);timeEl=n;r==2&&$(`#${n}_deviceState`).empty();curId>-1?(dsDivs[curId]=[],dsPaths[curId]=[],page[curId]=0,setChart(n,curId,r),clearInterval(uiTime),clearInterval(dataTime),t=(i.UI<1?1:i.UI)*1e3,uiTime=setInterval(()=>{run||setChart(n,curId,r,!0)},t),t=(i.Second<1?1:i.Second)*1e3,dataTime=setInterval(()=>{run||setChart(n,curId,r)},t)):(kanBanProductItemsEl.forEach(n=>{for(var t in n)~t.indexOf("_timer")&&stopScrollTable(n,t,t.replace("_timer",""))}),clearInterval(uiTime),clearInterval(dataTime))}});$(".content").on("click",".fullScreenBtn",function(){var t=`#${$(this).attr("target")}`,n;$(t).toggleClass("panel-fullscreen");n=$(t).hasClass("panel-fullscreen");$(this).toggleClass("glyphicon-fullscreen glyphicon-repeat").prop("title",n?"还原":"全屏放大");n?$(this).addClass("fsb"):$(this).removeClass("fsb");fullScreen(n);n||kanBanProductItemsEl.forEach(n=>{for(var t in n)~t.indexOf("_timer")&&stopScrollTable(n,t,t.replace("_timer",""))})});setInterval(()=>{$(`#${timeEl}_time`).length>0&&$(`#${timeEl}_time`).text(getFullTime()),$(`#${carouselFlag}_time`).length>0&&$(`#${carouselFlag}_time`).text(getFullTime())},1e3);n=function(){$("#kanBanLength").val(($("#kanBanRow").val()>>0)*($("#kanBanCol").val()>>0));let n=$(this).val()>>0;n==0&&(n=1,$(this).val(n));n>15&&(n=15,$(this).val(n));const t=$("#kanBanType").val()>>0;t==3&&initColSet(!1)};$("#kanBanRow").on("input",n);$("#kanBanCol").on("input",n);$("#kanBanList").on("select2:select",function(){var t=$(this).val(),n,i,r;t&&(n=kanBans[t],$("#kanBanName").val(n.Name),$("#kanBanOrder").val(n.Order),$("#isShow").iCheck(n.IsShow?"check":"uncheck"),$("#setKanBan .stateEl").addClass("hidden"),$("#setKanBan .stateEl1").addClass("hidden"),$("#setKanBan .productEl").addClass("hidden"),$("#kanBanUI").val(n.UI),$("#kanBanSecond").val(n.Second),n.Type==2?($("#kanBanRow").val(n.Row),$("#kanBanCol").val(n.Col),$("#kanBanContentCol").val(n.ContentCol),$("#kanBanLength").val(n.Length),$("#setKanBan .stateEl").removeClass("hidden"),scriptIndexTmp=[],scriptTmp=[],n.ContentCol>2&&($("#kanBanItemColNameDiv").removeClass("hidden"),i=getFirstOrders(n.ContentCol),r=`（首列：${i.join()},不为空的列名）`,$("#kanBanContentColDesc").text(r),initColName(!0)),$("#kanBanScript").html("")):n.Type==3&&($("#kanBanRow").val(n.Row),$("#kanBanCol").val(n.Col),$("#setKanBan .productEl").removeClass("hidden"),initColSet(!0),initProduct(n.Type),showPreProduct()),$("#deviceSelect").val(n.DeviceIdList).trigger("change"))});$("#deviceSelect").on("select2:select",function(){var n=$(this).val();~n.indexOf("0")&&$(this).val(0).trigger("change")}).on("change",function(){let n=$(this).val();if(isStrEmptyOrUndefined(n)){scriptIndexTmp=[];scriptTmp=[];$("#kanBanScript").html("");return}var t=[];n.length==1&&~n.indexOf("0")&&(n=deviceTmp.map(n=>n.Id).filter(n=>n));const i=$("#kanBanType").val()>>0;i==2&&(n.forEach(n=>{if(n!=0){var i=deviceTmp[n].ScriptId;~t.indexOf(i)||t.push(i);~scriptIndexTmp.indexOf(i)?existArrayObj(scriptTmp[i].ds,"id",n)||scriptTmp[i].ds.push({id:n,code:deviceTmp[n].Code}):(scriptIndexTmp.push(i),scriptTmp[i]={sId:i,sName:deviceTmp[n].ScriptName,ds:[{id:n,code:deviceTmp[n].Code}]})}}),scriptIndexTmp=t,scriptTmp.forEach(t=>{const i=t.sId;if(~scriptIndexTmp.indexOf(i)){const t=scriptTmp[i].ds.map(n=>n.id);t.forEach(t=>{~n.indexOf(t)||removeArray(scriptTmp[i].ds,"id",t)});scriptTmp[i].ds.length==0&&delete scriptTmp[i]}else scriptTmp[i].div&&$(`#${scriptTmp[i].div}`).remove(),delete scriptTmp[i]}),getScriptList())});i=new Promise(n=>getDevice(n));r=new Promise(n=>getKanBanList(n));Promise.all([i,r]).then(n=>{const i=n[0],t=n[1];t.length>0&&$("#kanBanList").trigger("select2:select")});$("#kanBanType").on("change",function(){const n=$(this).val(),i=$(this).find(`[value=${n}]`).css("color");$(this).css("color",i);$("#setKanBan .stateEl").addClass("hidden");$("#setKanBan .productEl").addClass("hidden");const t=kanBans.filter(t=>t.Type==n&&t.Id!=0);let r=0;const u=t.reduce((t,i)=>i.Id!==0&&i.Type==n?`${t}<option value=${i.Id}>${++r}-${i.Name}</option>`:"","");if($("#kanBanList").html(u),t.length>0){const n=t[0];$("#kanBanList").val(n.Id).trigger("select2:select")}});$("#kanBanItemColSet").on("input",".kb_item_col2",function(){let n=$(this).val()>>0;const r=$(this).attr("id");let t=0;$("#kanBanItemColSet .kb_item_col2").each((n,i)=>{let u=$(i).attr("id");r!=u&&(t+=$(i).val()>>0)});const i=100-t;n>i&&(n=i,$(this).val(n));initColSet(!1)});$("#kanBanContentCol").on("input",function(){const n=$(this).val();let t="";if(n>2){var i=getFirstOrders(n);t=`（首列顺序：${i.join()}）`;$("#kanBanItemColNameDiv").removeClass("hidden");initColName(!1)}else $("#kanBanItemColNameDiv").addClass("hidden");$("#kanBanContentColDesc").text(t)});$("#kanBanScript").on("input",".order",function(){const i=$(this).closest("tr"),n=$("#kanBanContentCol").val()>>0,t=getMaxSet(n);onInput(this,5,2,1,t)});$("#kanBanScript").on("input",".subOrder",function(){const n=$(this).closest("tr");onInput(this,5,2,1,5)});$("#kanBanItems").on("click",".itemAddBtn",function(){const t=$(this),n=$(t).attr("value").trim()>>0,f=t.attr("name"),i=colSet[0].chose.length,e=$("#kanBanList").val(),r=kanBans[e],o=kanBanItems[r.Type]?kanBanItems[r.Type]:[],u=o.filter(t=>t.Item==n);if(u.length>0){const t=u[0];kanBanItemsTmp.push({Item:n,Col:0,Order:i,Shifts:0,Hour:0,Min:0,FieldList:t.FieldList});colSet[0].chose.push({name:f,isTable:t.Display,val:n,col:0,order:i,height:-1,dOrder:0,dField:t.FieldList.length?t.FieldList[0].Field:""});showPreProduct()}});$("#kanBanProductDetail").on("click",".itemDelBtn",function(){const t=$(this).closest(".kb_div1"),r=$(t).attr("value")>>0;var n=($(t).find(`.C_${r}`).val()>>0)-1,i=($(t).find(`.O_${r}`).val()>>0)-1;removeArray(kanBanItemsTmp,"Col",n,"Order",i);kanBanItemsTmp.forEach(t=>{t.Col==n&&t.Order>=i&&t.Order--});removeArray(colSet[n].chose,"col",n,"order",i);colSet[n].chose=colSet[n].chose.sort(sortOrder);for(let t=0,i=colSet[n].chose.length;t<i;t++)colSet[n].chose[t].order=t;showPreProduct()});$("#kanBanProductDetail").on("click",".upTr",function(){const i=$(this).closest(".kb_div1"),r=$(i).attr("value")>>0,n=($(i).find(`.C_${r}`).val()>>0)-1,t=($(i).find(`.O_${r}`).val()>>0)-1;colSet[n]&&colSet[n].chose&&(colSet[n].chose[t].order=t-1,colSet[n].chose[t-1].order=t,colSet[n].chose=colSet[n].chose.sort(sortOrder));var u=kanBanItemsTmp.filter(i=>i.Col==n&&i.Order==t),f=kanBanItemsTmp.filter(i=>i.Col==n&&i.Order==t-1);kanBanItemsTmp.forEach(n=>{n==u[0]&&(n.Order=t-1),n==f[0]&&(n.Order=t)});kanBanItemsTmp=kanBanItemsTmp.sort(firstBy("Col").thenBy("Order"));showPreProduct()});$("#kanBanProductDetail").on("click",".downTr",function(){const i=$(this).closest(".kb_div1"),r=$(i).attr("value")>>0,n=($(i).find(`.C_${r}`).val()>>0)-1,t=($(i).find(`.O_${r}`).val()>>0)-1;colSet[n]&&colSet[n].chose&&(colSet[n].chose[t].order=t+1,colSet[n].chose[t+1].order=t,colSet[n].chose=colSet[n].chose.sort(sortOrder));var u=kanBanItemsTmp.filter(i=>i.Col==n&&i.Order==t),f=kanBanItemsTmp.filter(i=>i.Col==n&&i.Order==t+1);kanBanItemsTmp.forEach(n=>{n==u[0]&&(n.Order=t+1),n==f[0]&&(n.Order=t)});kanBanItemsTmp=kanBanItemsTmp.sort(firstBy("Col").thenBy("Order"));showPreProduct()});$("#kanBanProductDetail").on("change",".colSet",function(){var r;const n=$(this).closest(".kb_div1"),t=$(n).attr("value")>>0,u=($(n).find(`.C_${t}`).attr("old")>>0)-1,p=($(n).find(`.O_${t}`).attr("old")>>0)-1,f=($(n).find(`.C_${t}`).val()>>0)-1,i=($(n).find(`.O_${t}`).val()>>0)-1,h=$(n).find(`.DO_${t}`).val(),c=$(n).find(`.DF_${t}`).val(),l=$(n).find(`.N_${t}`).val().trim(),o=$(n).find(`.H_${t}`).val(),a=$(n).find(`.Shift_${t}`).val()>>0,v=$(n).find(`.Hour_${t}`).val()>>0,y=$(n).find(`.Min_${t}`).val()>>0;if(u==f)colSet[f].chose.forEach(n=>{n.order==i&&(n.name=l,n.dField=c,n.dOrder=h,n.height=isStrEmptyOrUndefined(o)?-1:o>>0)}),r=kanBanItemsTmp.filter(n=>n.Col==f&&n.Order==i),kanBanItemsTmp.forEach(n=>{n==r[0]&&(n.Shifts=a,n.Hour=v,n.Min=y)});else{var r=colSet[u].chose.filter(n=>n.order==i),e=kanBanItemsTmp.filter(n=>n.Col==u&&n.Order==i),s=i;r.length&&(removeArray(colSet[u].chose,"order",i),r[0].col=f,r[0].order=colSet[f].chose.length,colSet[f].chose.push(r[0]),s=r[0].order);e.length&&(removeArray(kanBanItemsTmp,"Col",u,"Order",i),e[0].Col=u,e[0].Order=s,kanBanItemsTmp.push(e[0]))}kanBanItemsTmp=kanBanItemsTmp.sort(firstBy("Col").thenBy("Order"));showPreProduct()});$("#kanBanProductDetail").on("click",".sUpTr",function(){const n=$(this).closest(".kb_div1"),i=$(n).attr("value")>>0,f=($(n).find(`.C_${i}`).val()>>0)-1,e=($(n).find(`.O_${i}`).val()>>0)-1,r=$(this).closest("tr"),t=$(r).attr("value")>>0,u=$(r).find(`.Or_${t}`).attr("value")>>0;var o=kanBanItemsTmp.filter(n=>n.Col==f&&n.Order==e);kanBanItemsTmp.forEach(n=>{if(n==o[0]){n.FieldList.forEach(n=>{n.XvHao==t&&(n.Order=u-1),n.XvHao==t-1&&(n.Order=u)});var i=n.FieldList.filter(n=>n.Order).sort(firstBy("Order")),r=n.FieldList.filter(n=>!n.Order).sort(firstBy("XvHao"));n.FieldList=i.concat(r);n.FieldList.forEach((n,t)=>{n.XvHao=t})}});showPreProduct()});$("#kanBanProductDetail").on("click",".sDownTr",function(){const n=$(this).closest(".kb_div1"),i=$(n).attr("value")>>0,f=($(n).find(`.C_${i}`).val()>>0)-1,e=($(n).find(`.O_${i}`).val()>>0)-1,r=$(this).closest("tr"),t=$(r).attr("value")>>0,u=$(r).find(`.Or_${t}`).attr("value")>>0;var o=kanBanItemsTmp.filter(n=>n.Col==f&&n.Order==e);kanBanItemsTmp.forEach(n=>{if(n==o[0]){n.FieldList.forEach(n=>{n.XvHao==t&&(n.Order=u+1),n.XvHao==t+1&&(n.Order=u)});var i=n.FieldList.filter(n=>n.Order).sort(firstBy("Order")),r=n.FieldList.filter(n=>!n.Order).sort(firstBy("XvHao"));n.FieldList=i.concat(r);n.FieldList.forEach((n,t)=>{n.XvHao=t})}});showPreProduct()});$("#kanBanProductDetail").on("change",".cb",function(){var r=$(this).prop("checked"),i;const n=$(this).closest(".kb_div1"),t=$(n).attr("value")>>0,u=($(n).find(`.C_${t}`).val()>>0)-1,f=($(n).find(`.O_${t}`).val()>>0)-1,e=$(this).closest("tr"),o=$(e).attr("value")>>0;i=kanBanItemsTmp.filter(n=>n.Col==u&&n.Order==f);kanBanItemsTmp.forEach(n=>{var u,t,f;n==i[0]&&(u=n.FieldList.length,n.FieldList.forEach(n=>{n.XvHao==o&&(n.Order=r?u+1:0)}),t=n.FieldList.filter(n=>n.Order).sort(firstBy("Order")),t.forEach((n,t)=>{n.Order=t+1}),f=n.FieldList.filter(n=>!n.Order).sort(firstBy("XvHao")),n.FieldList=t.concat(f),n.FieldList.forEach((n,t)=>{n.XvHao=t}))});showPreProduct()});$("#kanBanProductDetail").on("change",".fieldSet",function(){const n=$(this).closest(".kb_div1"),i=$(n).attr("value")>>0,r=($(n).find(`.C_${i}`).val()>>0)-1,u=($(n).find(`.O_${i}`).val()>>0)-1,f=$(this).closest("tr"),t=$(f).attr("value")>>0,e=$(n).find(`.Com_${t}`).val(),o=$(n).find(`.Wid_${t}`).val(),s=$(n).find(`.Func_${t}`).val(),h=$(n).find(`.Pre_${t}`).val(),c=$(n).find(`.Suf_${t}`).val();var l=kanBanItemsTmp.filter(n=>n.Col==r&&n.Order==u);kanBanItemsTmp.forEach(n=>{n==l[0]&&n.FieldList.forEach(n=>{n.XvHao==t&&(n.Column=e,n.Width=o,n.Func=unescape(s),n.Pre=h,n.Suffix=c)})})})}function initColName(n=true){let t=$("#kanBanContentCol").val()>>0;const u=$("#kanBanList").val(),i=kanBans[u];i&&(n&&(t=i.ContentCol),n&&(colName=i.ColName?i.ColName.split(","):[]));let r=colName.length;if(t>r)for(let n=colName.length,i=t;n<i;n++)colName[n]||(colName[n]="");else colName=sliceArray(colName,t);var f=Array.from($("#kanBanItemColName .kb_item_col2").map((n,t)=>t.value));for(let n=0,t=colName.length;n<t;n++){const t=f[n];t&&(colName[n]=t)}if(n){let n="";for(let i=0;i<t;i++){const t=colName[i];n+=`<div class="input-group kb_item_div1">
                    ...
                    </div>`);$("#kanBanItemColName").append(n)}else{for(let n=r-1;n>=t;n--)$(`#kanBanItemColName${n}`).closest(".kb_item_div1").addClass("rm");$("#kanBanItemColName .rm").remove()}}function initColSet(n=true){var f,e,u,o,s,h,c,l;let t=$("#kanBanCol").val()>>0;const a=$("#kanBanList").val(),i=kanBans[a];i&&n&&(t=i.Col,colSet=i.ColSet?JSON.parse(i.ColSet):[],kanBanItemsTmp=i.ItemList?i.ItemList:[],f=kanBanItemsTmp.filter(n=>n.Col==0),f.length&&(e=selectMany(colSet,"chose"),kanBanItemsTmp.forEach(n=>{var t,i,r;n.Col==0&&(t=e.filter(t=>t.val==n.Item),t.length&&(n.Col=t[0].col,n.Order=t[0].order));n.FieldList.forEach((n,t)=>{n.XvHao=t});i=n.FieldList.filter(n=>n.Order).sort(firstBy("Order"));r=n.FieldList.filter(n=>!n.Order).sort(firstBy("XvHao"));n.FieldList=i.concat(r)}),kanBanItemsTmp=kanBanItemsTmp.sort(firstBy("Col").thenBy("Order"))));let r=colSet.length;if(t>r)for(let n=colSet.length,i=t;n<i;n++)colSet[n]||(colSet[n]={width:0,chose:[]});else{for(let n=r-1;n>=t;n--){let t=colSet[0].chose.length;if(t)for(u=0,o=colSet[n].chose.length;u<o;u++){const i=colSet[n].chose[u];s=i.col;h=i.order;i.col=0;i.order=t++;colSet[0].chose.push(i);c=kanBanItemsTmp.filter(n=>n.Col==s&&n.Order==h);kanBanItemsTmp.forEach(n=>{n==c[0]&&(n.Col=i.col,n.Order=i.order)})}}colSet=sliceArray(colSet,t);kanBanItemsTmp=kanBanItemsTmp.sort(firstBy("Col").thenBy("Order"))}if(!n){l=Array.from($("#kanBanItemColSet .kb_item_col2").map((n,t)=>({width:t.value>>0})));for(let n=0,t=colSet.length;n<t;n++){const t=l[n];t&&(colSet[n].width=t.width)}}if(n){let n="";for(let i=0;i<t;i++){const t=colSet[i];n+=`<div class="input-group kb_item_div1">
                    <span class="text-bold input-group-addon kb_item_col1">列${i+1}：</span>
                    ...
                    <span class="input-group-addon kb_item_col3">%</span>
                    ...
                    </div>`,"");$("#kanBanItems").html(u)}function showPreProduct(){$("#kanBanProduct h3").text($("#kanBanName").val());const n=$("#kanBanType").val(),t=kanBanItems[n]?kanBanItems[n]:[],i=colSet.length,r=colSet.reduce((n,r)=>{r.chose=r.chose.sort(sortOrder);const u=r.chose.length,f=u>1?5:4,e=u>1?"":" hidden",o=r.chose.reduce((n,r,o)=>{const h=t.filter(n=>n.Item==r.val),s=kanBanItemsTmp.filter(n=>n.Col==r.col&&n.Order==r.order);if(!h.length||!s.length)return n;const l=h[0],y=l.FieldList,c=s[0].FieldList,a=c.filter(n=>n.Order).length,v=a>1,p=c.reduce((n,t,i)=>{const u=y.filter(n=>n.Field==t.Field),o=u.length?u[0].Column:"",f=u.length?u[0].DataType:"",w=!!FieldFunc[f],b=u.length?u[0].Special:"",k=w&&!!FieldFunc[f][b],e=t.Column?t.Column:"",s=t.Color?t.Color:"",h=t.Width?t.Width:"",c=t.Pre?t.Pre:"",l=t.Suffix?t.Suffix:"",p=t.Func?t.Func:"";return n+`<tr role="row" style="background-color: #f9f9f9;" value=${t.XvHao}>
                        <td style="padding: 4px; border: 1px solid gray;"><input type="checkbox" class="cb Cho_${i}" ${t.Order?"checked":""} value="0"></td>
                        ...
                        <td style="padding: 4px; border: 1px solid gray;" class="form-inline">
                            <input style="width: auto;${s?`color:${s};`:""}" class="form-control text-center no-padding fieldSet Com_${i}" old=${e} value=${e?e:""}>
                            <img class="mColorpicker Color_${i}"><i style="cursor: pointer;" class="CClear_${i}">清除</i>
                        </td>
                        <td style="padding: 4px; border: 1px solid gray;">
                            <input class="form-control text-center no-padding fieldSet Wid_${i}" placeholder="自动" old=${h} value=${h}>
                        </td>
                        <td style="padding: 4px; border: 1px solid gray;">
                            <button class="btn btn-primary btn-sm Func_${i}" old="${escape(p)}" value="${escape(p)}" onclick="showFunc(this, '${o}', '${f}', ${r.col}, ${r.order}, ${t.XvHao})">设置</button>
                        </td>
                        <td style="padding: 4px; border: 1px solid gray;">
                            <input class="form-control text-center no-padding fieldSet Pre_${i}" old=${c} value=${c}>
                        </td>
                        <td style="padding: 4px; border: 1px solid gray;">
                            <input class="form-control text-center no-padding fieldSet Suf_${i}" old=${l} value=${l}>
                        </td>
                        <td style="padding: 4px; border: 1px solid gray;" class="form-inline Or_${i}" value=${t.Order}>
                            <span class="glyphicon glyphicon-arrow-up pointer text-green${i==0?" hidden":""} ${v&&t.Order?"":" hidden"} sUpTr" aria-hidden="true" title="上移"></span>
                            <span class="glyphicon glyphicon-arrow-down pointer text-red${i+1==a?" hidden":""} ${v&&t.Order?"":" hidden"}   sDownTr" aria-hidden="true" title="下移"></span>
                        </td>
                    </tr>`},""),w=setOptions(shift,"Type");return`${n}<div class="kb_div1" style="height: ${r.height==-1?"auto":`${r.height}%`}" col="${r.col}" order="${r.order}" value="${r.val}">
                    <div class="kb_border1">
                        <div class="kb_border3" style="padding: 5px;">
                            <table class="table table-striped no-margin">
                                ...
                                <tbody>
                                    <tr role="row" class="odd">
                                        ...
                                        <td style="padding: 4px; border: 1px solid gray;" class="form-inline">
                                            排序
                                            <select class="form-control colSet DF_${r.val}" style="width: auto;">
                                                ${setOptionsWithKey(c,"Field","Column")}
                                            </select>
                                            <select class="form-control colSet DO_${r.val}" style="width: auto;">
                                        ...
                                        ${!h[0].BShifts&&!h[0].BDuration?"":`<tr role="row" style="background-color: #f9f9f9;">
                                        <td style="padding: 4px; border: 1px solid gray;" class="form-inline" colspan="${f}" >
                                            <div class="form-group form-inline">
                                            ${h[0].BShifts?`<label class="control-label">班制</label>
                                                <select class="form-control colSet Shift_${r.val}" value=${s.length?s[0].Shifts:0}>${w}</select>`:""}
                                            ${h[0].BDuration?`<label class="control-label">显示最近</label>
                                                <input class="form-control text-center colSet Hour_${r.val}" style="width: 70px;" value=${s.length?s[0].Hour:24} oninput="onInput(this, 3, 0, 0, 24);">
                                                <label class="control-label">小时</label>
                                                <input class="form-control text-center colSet Min_${r.val}" style="width: 70px;" value=${s.length?s[0].Min:0} oninput="onTimeLimitInput(this);">
                            ...
                            <br>
                            ${c.length?`<table class="table table-striped no-margin">
                                <thead>
                                    <tr role="row">
        ...
        </div>`;if(carouselChose.length){const t=carouselChose.reduce((n,t)=>`${n}${carouselTmp[t.id].carouselLi}`,""),i=carouselChose.reduce((n,t)=>`${n}${carouselTmp[t.id].carouselTab}`,"");_carouselBox=n.format(t,i);carouselChose.forEach(n=>{page[n.id]=0,dsDivs[n.id]=[],dsPaths[n.id]=[]});carouselId=0;carouselIndex=0;$("#fullScreenCarousel").empty().append(_carouselBox);$("#fullScreenCarousel").find("ol li:first").addClass("active");$("#fullScreenCarousel").find(".item:first").addClass("active");$("#fullScreenCarousel").animate({top:0,opacity:1},1e3,"linear",()=>{});carouselFlag=$("#fullScreenCarousel .item:first").attr("id");setFullScreenCarousel(carouselFlag);$("#carousel-example-generic").carousel({interval:1e4}).off("slide.bs.carousel").on("slide.bs.carousel",n=>{if(carouselFlag=n.relatedTarget.id,carouselId=carouselFlag.replace("carousel_kanBan_",""),kanBans[carouselId]){if($(`#fullScreenCarousel .ftitle`).length&&$(`#fullScreenCarousel .ftitle`).text(` - ${kanBans[carouselId].Name}`),carouselFlag=="")return;setChart(carouselFlag,carouselId,kanBans[carouselId].Type,!0)}}).on("mouseenter",()=>$("#carousel-example-generic .isShow").removeClass("hidden")).on("mouseleave",()=>$("#carousel-example-generic .isShow").addClass("hidden")).on("mousemove",()=>{$("html").css({cursor:"default"}),checkMouseMove&&clearTimeout(checkMouseMove),checkMouseMove=setTimeout(function(){console.log("hide");$("html").css({cursor:"none"});$("#carousel-example-generic").trigger("mouseleave")},5e3)})}}function setFullScreenCarousel(){(clearInterval(carouselTime),carouselFlag!="")&&(carouselTime=setInterval(()=>{if(carouselId=carouselFlag.replace("carousel_kanBan_",""),kanBans[carouselId]){if($(`#fullScreenCarousel .ftitle`).text(` - ${kanBans[carouselId].Name}`),carouselFlag=="")return;setChart(carouselFlag,carouselId,kanBans[carouselId].Type,!0)}},5e3))}function cancelFullScreenCarousel(){kanBanProductItemsEl.forEach(n=>{for(var t in n)~t.indexOf("_timer")&&stopScrollTable(n,t,t.replace("_timer",""))});checkMouseMove&&clearTimeout(checkMouseMove);clearInterval(carouselTime);carouselFlag="";$("#carousel-example-generic").carousel("pause").off("slide.bs.carousel mouseenter mouseleave");$("#fullScreenCarousel").animate({top:"-100%",opacity:0},1e3,"linear",function(){$(this).empty();fullScreen(!1)})}function setChart(n,t,i,r=false){var u,f;if(!run){run=!0;u=`#${n}`;const e=page[t]?page[t]:0;f={opType:509,opData:JSON.stringify({qId:t,page:e})};ajaxPost("/Relay/Post",f,e=>{var s,a,o,g,st,ht,ct,nt,lt,it,at,w,rt,ut,ni,l,h,ti,ii,p,ri,ft,b,et,k,ui,fi,ei,ot,d;run=!1;const v="text-red",y="text-gray";if(g=new Date,tt||(tt=e.time,tn=g),tt!=e.time&&(tt=e.time,tn=g),e.errno==0?(st=$(`${u}_time`).text(),ht=e.time,Math.abs(dateDiffSec(ht,st))>60?($(`${u}_time`).toggleClass(v),$(`${u}_time`).toggleClass(y)&&$(`${u}_time`).removeClass(y)):($(`${u}_time`).toggleClass(y),$(`${u}_time`).hasClass(v)&&$(`${u}_time`).removeClass(v))):($(`${u}_time`).hasClass(v)||$(`${u}_time`).addClass(v),$(`${u}_time`).hasClass(y)&&$(`${u}_time`).removeClass(y),run=!1),o=e.data,i==1){if(e.errno!=0)return;ct=setPieChart(`${u}_detail`,"Rate",o.AllProcessRate,"#0099ff");$(`${u}_yxTime`).text(codeTime(o.RunTime));$(`${u}_jgTime`).text(codeTime(o.ProcessTime));$(`${u}_xzTime`).text(codeTime(o.IdleTime));$(`${u}_lly`).text(ct);nt=`${o.MaxUse}台`;lt=setPieChart(`${u}_max`,`Max
${nt}`,o.MaxUseRate,"#00a65a");$(`${u}_maxTs`).text(`${o.MaxSimultaneousUseRate}台`);$(`${u}_maxTNum`).text(nt);$(`${u}_maxSyl`).text(lt);it=`${o.MinUse}台`;at=setPieChart(`${u}_min`,`Min
${it}`,o.MinUseRate,"#f39c12");$(`${u}_minTs`).text(`${o.MinSimultaneousUseRate}台`);$(`${u}_minTNum`).text(it);$(`${u}_minSyl`).text(at);var c=o.AllDevice,vt=o.NormalDevice,yt=o.ProcessDevice,pt=o.IdleDevice,wt=o.FaultDevice,bt=o.ConnectErrorDevice;for(setPieChart(`${u}_state_all`,`${c}台`,c/c,"green","总设备"),setPieChart(`${u}_state_normal`,`${vt}台`,vt/c,"blue","正常运行"),setPieChart(`${u}_state_process`,`${yt}台`,yt/c,"#0099ff","加工中"),setPieChart(`${u}_state_idle`,`${pt}台`,pt/c,"#cc33ff","闲置中"),setPieChart(`${u}_state_fault`,`${wt}台`,wt/c,"red","故障中"),setPieChart(`${u}_state_error`,`${bt}台`,bt/c,"#ff00cc","连接异常"),rt=o.UseCodeList||[],a="",s=0,w=rt.length;s<w;s++)a+=`<div style="border:1px solid;padding:5px 10px;margin:3px;border-radius:20%;background:#bfa;font-weight:bold">${rt[s]}</div>`;$(`${u}_dev`).empty().append(a||'<label style="color:red;font-size:18px">无<\/label>');var kt=o.SingleProcessRate,dt=[],gt=[];for(s=0,w=kt.length;s<w;s++)ut=kt[s],dt.push(ut.Code),gt.push((ut.Rate*100).toFixed(2));ni={tooltip:{trigger:"axis",formatter:"{a}<br/>{b} : {c}%",axisPointer:{type:"shadow"}},grid:{left:"3%",right:"3%",top:"6%",bottom:"8%",containLabel:!0},xAxis:{type:"category",axisLine:{onZero:!1},data:dt},yAxis:{name:"百分比",type:"value",scale:!0,axisLabel:{show:!0,formatter:"{value}%"}},series:{name:"利用率",type:"bar",areaStyle:{normal:{}},barWidth:"60%",label:{show:!0,position:"top",formatter:"{c}%"},itemStyle:{color:n=>n.value<30?"red":"green"},data:gt},dataZoom:[{type:"inside"},{handleIcon:_handleIcon,handleSize:"80%",handleStyle:{color:"#fff",shadowBlur:3,shadowColor:"rgba(0, 0, 0, 0.6)",shadowOffsetX:2,shadowOffsetY:2}}]};echarts.init($(`${u}_bar`)[0]).setOption(ni,!0);$(u).off("resize").on("resize",function(){clearTimeout(this.time);this.time=setTimeout(()=>{for(var t=$(this).find(".chart"),n=0,i=t.length;n<i;n++)echarts.init(t[n]).resize()},200)});l=o.ProductionList;const n=l.reduce((n,t)=>`${n}<td class="text-blue">${t.Code}</td>`,""),t=l.reduce((n,t)=>`${n}<td>${t.DayTotal}</td>`,""),i=l.reduce((n,t)=>`${n}<td>${t.DayQualified}</td>`,""),r=l.reduce((n,t)=>`${n}<td>${t.DayUnqualified}</td>`,""),f=l.reduce((n,t)=>`${n}<td>${t.DayQualifiedRate}%</td>`,""),h=l?l.filter(n=>n.DayTotal>0).length:0;$(`${u}_devs`).text(h);$(`${u}_pros`).text(o.DayTotal);$(`${u}_heGes`).text(o.DayQualified);$(`${u}_liePians`).text(o.DayUnqualified);$(`${u}_rates`).text(`${o.DayQualifiedRate}%`);$(`${u}_devOps`).find("td:not(:first)").remove().end().append(n);$(`${u}_proOps`).find("td:not(:first)").remove().end().append(t);$(`${u}_heGeOps`).find("td:not(:first)").remove().end().append(i);$(`${u}_liePianOps`).find("td:not(:first)").remove().end().append(r);$(`${u}_rateOps`).find("td:not(:first)").remove().end().append(f)}else if(i==2){if(h=`${u}_deviceState`,e.errno!=0){dsDivs[t].forEach(n=>{$(`#${n}`).addClass("hi")});$(`${h} .hi`).css("display","none").removeClass("hi");return}ti=`<div id="{0}" class="kb_device_div1 ds" style="display:none;">
                    ...
                    <div style="top: 2%; height: 65%; position: relative;" id="{0}_detail"></div>
                            ...
                            </table>`}}return e}var a,v,f,o;if(n)for(l=n.length,s=0;s<l;s++)a=n[s],dsPaths[t][a.name]||(dsPaths[t][a.name]=a.path);const tt=Object.keys(dsPaths[t]).length;if(!(tt<=0)){for(v=[],c.forEach(n=>v[n.Id]=1),s=0;s<i;s++){const n=`${u}_ds${s}`;if(f=c[s],f){o="red";switch(f.DeviceStateStr){case"加工中":o="#00A65C";break;case"待机":case"流程升级中":case"固件升级中":case"设备重启中":case"连接正常":o="#f47920";break;case"已报修":case"已确认":case"维修中":o="red";break;default:o="gray"}const i=it(f.Data,e.cCol,e.cName);$(`${n}_img`).attr("src",dsPaths[t][f.Icon]);$(`${n}_code`).text(f.Code);$(`${n}_mc`).text(`${f.CategoryName}-${f.ModelName}`);$(`${n}_ws`).text(f.SiteName);$(`${n}_state`).css("background",o);$(`${n}_detail`).html(i);$(`${n}`).addClass("sh")}else $(`${n}`).addClass("hi")}$(`${h} .sh`).css("display","block").removeClass("sh");$(`${h} .hi`).css("display","none").removeClass("hi");$(`${u}_jg`).text(e.jg);$(`${u}_xz`).text(e.xz);$(`${u}_zc`).text(e.zc);$(`${u}_gz`).text(e.gz);$(`${u}_wlj`).text(e.wlj);$(`${u}_sum`).text(e.sum);var y=$(".ds"),p=y.length>0?($(y[0]).css("margin").replace("px","")>>0)*2:5,rt=($(h).css("padding").replace("px","")>>0)*2;if(e.row>0){var w=$(h).width(),b=Math.floor(w/(def+p)),ut=e.col>b?b:e.col,k=Math.floor(w/ut)-p,ft=$(window).height();$(`${u}`).css("height",`${ft}px`);var d=$(h).closest(".dsDiv").height(),g=Math.floor(d/def),et=e.row>g?g:e.row,nt=Math.floor(d/et)-rt;$(".ds").css("flex-basis",`${k<def?def:k}px`).css("height",`${nt<def?def:nt}px`)}r&&(page[t]=++e.cp)}};const v=[];for(s=0;s<l;s++)b=c[s],dsPaths[t][b.Icon]||~v.lastIndexOf(b.Icon)||v.push(b.Icon);if(v.length>0){f={type:fileEnum.Device,files:JSON.stringify(v)};f.dir="";for(et in fileEnum)if(fileEnum[et]==f.type){f.dir=et;break}if(isStrEmptyOrUndefined(f.dir))return void layer.msg("文件类型不存在！");getFilePath(f,ft,0)}else ft()}else if(i==3){if(e.errno!=0)return;if(!e.data)return;var o=e.data,oi=e.colSet?JSON.parse(e.colSet):[],si=`${u}_productDiv`,p=$(window).height(),hi=$(`${u} .kb_title1`).closest("div").height()>>0;for($(`${si}`).css("height",`${p-hi}px`),k=0,ui=e.items.length;k<ui;k++){const t=e.items[k],h=t.Item,r=kanBanItems[i][h],f=oi[t.Col].chose[t.Order];if(t&&r&&f){const s=t.FieldList=t.FieldList?t.FieldList.filter(n=>n.Order).sort(firstBy("Order")):[];if(s&&s.length){const l=`${t.Item}_${t.Col}_${t.Order}`,i=`${u}_${t.Col}_${t.Order}`,h=f.dOrder=="0"?"asc":"desc";if(!o.hasOwnProperty(l))continue;const e=o[l].sort(firstBy(f.dField,h));$(`${i}`).closest(".kb_border2").find(".tt1").text(f.name);$(`${i}`).closest(".kb_border2").find(".tt2").text(e.length);addOrderData(e);let c=$(`${i} tbody tr`);if(r.Name=="操作工日进度表"&&(d=1),f.isTable=f.isTable?f.isTable:0,f.isTable==0){if(c.length==0){const n=getItemTrs(e,r,t,h);$(`${i} tbody`).html(n)}else{const n=$(`${i} tr[class='max']`),f=$(n).attr("order")>>0;let u=!1;if(c.each((n,t)=>{const i=$(t).attr(trFlag),r=e.filter(n=>n[trFlag]==i);r.length==0&&($(t).remove(),u=!0)}),f==0&&(u=!0),u&&c.length>0&&($(`${i} tbody`).html(getItemTrs(e,r,t,h)),c=$(`${i} tbody tr`)),!u){const u=e.length-c.length;let o=[];if(o=h=="asc"?e.filter(n=>n.XvHao<=f):sliceArray(e,e.length,u-1),r.Name=="操作工日进度表"&&(d=1),o.forEach(n=>{var t,l,c;const u=$(`${i} tr[${trFlag}=${n[trFlag]}]`),h=[];r.FieldList&&r.FieldList.forEach(n=>{h[n.Field]=n});const a=s.filter(n=>n.Field=="XvHao");for(a.length||s.unshift({Order:0,Column:"序号",Field:"XvHao",Width:"60"}),t=0,l=s.length;t<l;t++){const i=s[t];var o=n[i.Field],v=i.Pre?i.Pre:"",y=i.Suffix?i.Suffix:"",p=i.Width&&i.Width!=="auto"?`width: ${i.Width}px;`:`width: auto;`,f="",e="";if(h[i.Field]&&i.Func){const n=JSON.parse(i.Func);if(n.color==1?e=i.Color:n.color==2&&n.cons.every(n=>meetCondition(o,n.con,n.conv)?(e=n.color,!1):!0),n.special){const t=h[i.Field].DataType;o=FieldFunc[t][n.special].func(o)}n.bold&&(f="text-bold")}e=e?`color: ${e};`:`color: "";`;const r=`${v}${o}${y}`,l=n.XvHao,a=$(u.find("td")[t]).text(),w=$(u).attr("order")>>0;a!=r&&$(u.find("td")[t]).html(r);w!=l&&$(u).attr("order",l);c=$(u.find("td")[t]).hasClass(f);f&&!c?$(u.find("td")[t]).addClass(f):!f&&c&&$(u.find("td")[t]).removeClass(f);$(u.find("td")[t]).attr("style",p);$(u.find("td")[t]).attr("style",e)}}),e.length>c.length){n.removeClass("max");let t=[];t=h=="asc"?e.filter(n=>n.XvHao>f):sliceArray(e,u);r.Name=="操作工日进度表"&&(d=1);const o=t.reduce((n,r,u)=>{const f=$(`${i} tr[${trFlag}=${r[trFlag]}]`);if(f.length==0){const f=cols.reduce((n,t)=>{var i=t.data?r[t.data]:t.render(r),u=t.suffix?t.suffix:"";return n+`<td>${i}${u}</td>`},"");let i="";return(h=="asc"&&u+1==t.length||h=="desc"&&u==0)&&(i=` class="max"`),n+`<tr${i} order=${r.XvHao} ${trFlag}=${r[trFlag]}>${f}</tr>`}return n},"");h=="asc"?n.after(o):n.before(o)}else if(e.length<c.length){const n=getItemTrs(e,r,t,h);$(`${i} tbody`).html(n)}}}const f=$(`${i}`).closest(".kb_border2").height(),l=$(`${i}`).closest(".kb_border2").find("h4").height(),o=f-l;$(`${i}`).closest(".table-responsive").css("height",`${Math.floor(o*100/f)}%`);fi=$(`${i} tr`).height();ei=o-$(`${i} tbody`).closest(".kb_item_tablebox").css("padding").replace("px","")*2;c=$(`${i} tbody tr`);ot=Math.floor(ei/fi);const u=`${n}_${t.Col}_${t.Order}_timer`;if(r.Name=="设备状态反馈"&&(d=1),c.length>ot&&!kanBanProductItemsEl[u])scrollTable(kanBanProductItemsEl,u,`${n}_${t.Col}_${t.Order}`);else if(c.length<=ot&&kanBanProductItemsEl[u]){const f=getItemTrs(e,r,t,h);$(`${i} tbody`).html(f);stopScrollTable(kanBanProductItemsEl,u,`${n}_${t.Col}_${t.Order}`)}}else f.isTable==1}}}}},0)}}function getItemTrs(n,t,i,r){const u=[];t.FieldList&&t.FieldList.forEach(n=>{u[n.Field]=n});const f=i.FieldList?i.FieldList.filter(n=>n.Order):[],e=f.filter(n=>n.Field=="XvHao");return e.length||f.unshift({Order:0,Column:"序号",Field:"XvHao",Width:"60"}),n.reduce((t,i,e)=>{const s=f.reduce((n,t,r)=>{var e=i[t.Field],h=t.Pre?t.Pre:"",c=t.Suffix?t.Suffix:"",l=t.Width&&t.Width!=="auto"?`width: ${t.Width}px;`:`width: auto;`,o="",f="",r,s;if(u[t.Field]&&t.Func){const n=JSON.parse(t.Func);if(n.color==1)f=t.Color;else if(n.color==2)for(r=0,s=n.cons.length;r<s;r++){const t=n.cons[r];if(meetCondition(e,t.con,t.conv)){f=t.color;break}}if(n.special){const i=u[t.Field].DataType;e=FieldFunc[i][n.special].func(e)}n.bold&&(o="text-bold")}return f=f?`color: ${f};`:``,n+`<td class="${o}" style='${l}${f}'>${h}${e}${c}</td>`},"");var o="";return(r=="asc"&&n.length==e+1||r=="desc"&&e==0)&&(o=` class="max"`),t+`<tr${o} order=${i.XvHao} ${trFlag}=${i[trFlag]}>${s}</tr>`},"")}function setPieChart(n,t,i,r,u){i=i||0;var e=`${(i*100).toFixed(2)}%`,f={color:["#cdcdcd",r],series:{type:"pie",center:["50%","50%"],radius:["65%","100%"],hoverAnimation:!1,animation:!1,silent:!0,label:{show:!0,position:"center",formatter:`${t}
${e}`,textStyle:{color:r,fontSize:18,fontWeight:700}},data:[1-i,i]}};return u&&(f.title={text:u,left:"center",textStyle:{color:r,fontSize:16}},f.series.center=["50%","57%"],f.series.radius=["57%","85%"]),echarts.init($(n)[0]).setOption(f,!0),e}var def=175,curId=-1,t=0,uiTime=0,dataTime=0,run=!1,page=[],timeEl="",deviceTmp,_carouselBox,_dataTableData,tt,tn,pTable;let dsDivs=[],dsPaths=[],carouselTmp=[],carouselChose=[],carouselIndex=0,carouselId=0;var carouselTime=0,carouselFlag="",scriptIndexTmp=[],scriptTmp=[],kanBans=[],kanBanItems=[],shift=[],kanBanItemsTmp=[],trFlag="XvHao";$(document).on("keydown",function(n){n.keyCode==27&&($(".content").find(".fullScreenBtn.fsb").click(),cancelFullScreenCarousel&&cancelFullScreenCarousel())});var kanBanProductItemsEl=[],kanBanProductItemsTables=[],tableSet=tableDefault(),_permissionList=[];let colName=[];let colSet=[];deviceTmp=[];_dataTableData={};let checkMouseMove;pTable=[];