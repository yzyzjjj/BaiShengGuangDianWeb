                            ...
                            <table class="table table-hover table-striped table-bordered" id="taskKanBanList">
                                <thead>
                                    <tr>
                                        ...
                                        <th rowspan="2" style="width: 82px">状态</th>${t}
                                    </tr>
                                    <tr>${'<th class="bg-blue">已入库<\/th><th>入库率<\/th><th>理论率<\/th><th>状态<\/th>'.repeat(n.Orders.length)}</tr>
                                </thead>
                                <tbody>${i}</tbody>
                            </table>
                        </div>
                      </div>`,r=n.Orders.sort(sortOrder),i=4,o=r.reduce((n,t,r)=>`${n}<th colspan="${i}"${r%2==0?'class="bg-gray"':""}>${t.Process}</th>`,""),s=f.reduce((n,t,u)=>{const o=t.Id,f={};t.Needs.forEach(n=>{f[n.Order]=n});const e=r.reduce((n,t)=>{const r=f[t.Order];return n+(r?`<td class="bg-blue">${r.DoneTarget}</td>
                           <td>${r.ActualRate}%</td>
                           <td>${r.TheoreticalRate}%</td>
                           <td>${r.ErrorStr}</td>`:i>0?`<td class="bg-blue short-slab"><i></td>`+(i>1?`<td class="short-slab"><i></td>`.repeat(i-1):``):``)},"");return`${n}<tr>
                        <td>${u+1}</td>
                        <td>${t.TaskOrder}</td>
                        <td>${t.Product}</td>
                        <td>${t.DeliveryTime.split(" ")[0]}</td>
                        <td>${t.ErrorStr}</td>${e}
                    </tr>`},""),h=e(o,s);$("#taskKanBanListBox").html(h).find("table").css("width","100%").find("th,td").css("padding","4px").end().find("th,td").css("border","1px solid gray").end().find("th,td").css("width","auto");var t=dataTableConfig(0);t.dom='<"pull-left"l><"pull-right"f>rt';t.bAutoWidth=!1;t.ordering=!1;t.paging=!1;t.searching=!1;t.aLengthMenu=[u];t.iDisplayLength=u;$("#taskKanBanList").DataTable(t);$("#taskKanBanListBox .DTFC_ScrollWrapper").css("height","auto");_tkbPage++;const c=Math.floor(n.Count/u);_tkbPage>c&&(_tkbPage=0)})}}const tableSet=tableDefault();let _currentTime=null,_tkbTime=null,_currentNav=null;let _workshopTrs=null,_workshopListTable=null;let _deviceTrs=null,_deviceListTable=null;let _deviceCategoryTrs=null,_deviceCategoryListTable=null,_deviceCategoryModel=!1;let _deviceModelTrs=null,_deviceModelListTable=null,_deviceCategoryCache=null;let _personTrs=null,_personListTable=null;let _personGradeTrs=null,_personGradeListTrs=null;let _processCodeTrs=null,_processCodeListTable=null;let _processCodeCategoryTrs=null,_processCodeCategoryListTable=null;let _processTrs=null,_processListTable=null;let _planTrs=null,_planListTable=null;let _planProcessCodeInfo=null;let _capacityTrs=null,_capacityListTrs=null;let _capacityDetailList=null,_capacityDetailListType=-1,capacityId=-1,currentCategoryId=-1;let _workOrderTrs=null,_workOrderTable=null;let _taskOrderTrs=null,_taskOrderTable=null;let _capacityNeed=[],_capacityNeedCurrentTaskId=0,_capacityNeedCurrentPId=0;let _capacityTaskProcessListInit=!1,_capacityTaskDetailList=null,_capacityTaskDeviceTrs=null,_capacityTaskOperatorTrs=null;let _capacityTasks=null;let _notArrangeTaskListTable=null,_pmcChildAutoWait=!1;let _pmcPreviewParams={};let _isGetPmcPreviewParams=!1;let _arrangeTask=null,_arrangeTaskListTable=null;let _taskOrders=[];let _pmcTaskLevelTrs=null;let _flowCardTrs=null,_flowCardListTable=null;let _addFlowCardPreview=!1;let _tkbTable=null;let _tkbPage=0;