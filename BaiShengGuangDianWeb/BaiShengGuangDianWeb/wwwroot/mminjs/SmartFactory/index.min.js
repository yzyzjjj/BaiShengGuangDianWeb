function pageReady(){$(".sidebar-mini").addClass("sidebar-collapse");$("#sendCardSTime,#sendCardETime,#pmcChildSTime,#pmcChildETime").val(getDate()).datepicker("update");$("#pmcChildDTime").val(getNowWeekRange((new Date).getDay()==0?7:(new Date).getDay()).end).datepicker("update");getNotArrangeTaskList();getArrangeTaskList();$("#workshopNavLi").one("click",getWorkshopList);$("#deviceNavLi").one("click",getDeviceWorkshopList);$("#pmcPersonNavLi").one("click",getPmcPersonList);$("#flowNavLi").one("click",getProcessCodeList);$("#processSetNavLi").one("click",getProcessOpList);$("#planNavLi").one("click",getPlanList);$("#workOrderNavLi").one("click",getWorkOrderList);$("#taskOrderNavLi").one("click",getTaskOrderList);$("#flowCardNavLi").one("click",()=>{const n=myPromise(5090),t=myPromise(5040),i=myPromise(5060);Promise.all([n,t,i]).then(n=>{const t='<option value="0">所有<\/option>';$("#flowCardTaskOrderSelect").html(`${t}${setOptions(n[0].datas,"TaskOrder")}`);$("#flowCardProcessCodeSelect").html(`${t}${setOptions(n[1].datas,"Code")}`);$("#flowCardPlanSelect").html(`${t}${setOptions(n[2].datas,"Product")}`);getFlowCardList()})});$("#pmcExpelProNavLi").one("click",()=>{getNotArrangeTaskList(),getArrangeTaskList()});$("#addProcessCodeBody,#addProcessCodeCategoryBody").on("click",".upTr",function(){const n=$(this).parents("tr"),t="#"+$(this).parents("tbody").attr("id"),i=n.prev();i.before(n);setAddProcessOpList(t)}).on("click",".delBtn",function(){const n="#"+$(this).parents("tbody").attr("id");$(this).parents("tr").remove();setAddProcessOpList(n)});$("#addPlanCapacity").on("change",function(){const n=n=>{const t=_tablesConfig(!1,n);t.columns=t.columns.concat([{data:"Process",title:"流程"},{data:"Category",title:"设备类型"},{data:null,title:"产能",render:n=>`<button class="btn btn-info btn-sm capacity-btn" value="${n.Id}" process="${n.ProcessId}" p="${n.Process}">查看</button>`}]);$("#addPlanCapacityList").DataTable(t)},t=$(this).val();t?myPromise(5560,{capacityId:t},!0).then(t=>n(t.datas)):n([])});$("#addPlanCapacityList").on("input",".rate",function(){$(this).val()>>0>100&&$(this).val(100)}).on("input",".minute,.second",function(){$(this).val()>>0>59&&$(this).val(59)}).on("click",".capacity-btn",function(){let t="qId",n=$(this).attr("list");n&&n!=0||(t="processId",n=$(this).attr("process"));var r=$(this).attr("p");const i={};i[t]=n;myPromise(5564,i,!0).then(n=>{n.Process=r,devicesOperatorsTable(n,!0,"planDevCapacitySetBox","planDevCapacitySetList","planPersonCapacitySetList")});$("#addCapacitySetBtn").addClass("hidden")});$("#addPlanProcess").on("change",function(n,t){const i=$(this).val(),r=myPromise(5530,{categoryId:i,menu:!0},!0),u=myPromise(5040,{categoryId:i},!0);Promise.all([r,u]).then(n=>{const i=n[1].datas;i.forEach(n=>_planProcessCodeInfo[n.Id]=n);const r=`<div class="temp form-group" style="border-bottom:1px solid #eee">
                ...
                ${p}
                    ...
                    <th></th><th></th><th></th><th>${'<th class="bg-gray text-red">型号<\/th><th class="bg-gray">需求<\/th><th class="bg-gray">现有<\/th><th class="bg-gray">班次<\/th><th><\/th><th class="bg-gray text-red">等级<\/th><th class="bg-gray">需求<\/th><th class="bg-gray">现有<\/th><th class="bg-gray">班次<\/th><th><\/th>'.repeat(t.length)}
                                ...
                                <div class="table-responsive1 mailbox-messages">
                                    <table class="table table-hover table-striped table-bordered" id="notArrangeTaskProcess">
                                        <thead>
                                            ...
                                            <tr>${'<th class="bg-blue">总计<\/th><th class="bg-green">库存<\/th><th class="bg-yellow">需生产<\/th><th>需投料<\/th><th class="bg-yellow">已完成<\/th><th>已投料<\/th>'.repeat(i)}</tr>
                                        </thead>
                                        <tbody>${r}</tbody>
                                    </table>
                                </div>
                    ...
                    </div>`,u=n.Orders.sort(sortOrder),o=u.reduce((n,t,i)=>`${n}<th colspan="6" ${i%2?"":'class="bg-gray"'}>${t.Process}</th>`,""),s=i.sort(sortOrder).reduce((n,t,i)=>{const o=t.Id,f=t.Needs.sort(sortOrder),r={};f.forEach(n=>{r[n.Order]=n});const e=u.reduce((n,t)=>{const i=r[t.Order];return i?`${n}
                        ...
                        <td>${t.Product}</td>${e}
                    </tr>`},""),h=e(o,u.length,s);$("#notArrangeTaskProcessBox").html(h).find("table").css("width","100%").find("th,td").css("padding","4px").end().find("th,td").css("border","1px solid black").end().find("tbody .bg-green").css("padding",0);var f=dataTableConfig(0);f.fixedHeaderColumn(!0,3,0);$("#notArrangeTaskProcess").DataTable(f);$("#notArrangeTaskProcessBox .DTFC_ScrollWrapper").css("height","auto");$("#setNotArrangeTaskProcessBtn").off("click").on("click",()=>{if(getPmcPreviewParams(),!_taskOrders.length)return layer.msg("请选择任务单");myPromise(5602,_taskOrders).then(n=>{updatePmcPreviewParams(i),r(n)})})};myPromise(5602,_taskOrders,!0,n).then(r)}}function getPmcPreviewList(){const n=getPmcPreviewParams(!0);if(!isStrEmptyOrUndefined(n))return layer.msg(n);if(_taskOrders.length==0)return"";myPromise(5604,!$("#notArrangeTaskProcessBox").html()?[]:_taskOrders,!0).then(n=>{n&&n.Cost&&n.Cost.length>0&&(_isGetPmcPreviewParams=!1);const r=(t,i)=>`<div class="form-group">
