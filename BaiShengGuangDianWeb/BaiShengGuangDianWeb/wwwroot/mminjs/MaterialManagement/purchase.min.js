function pageReady(){_permissionList[589]={uIds:["updateDepartmentBtn"]};_permissionList[590]={uIds:["finishInWareListBtn"]};_permissionList[591]={uIds:["citePurchaseListBtn"]};_permissionList[592]={uIds:["printPurchaseListBtn"]};_permissionList[593]={uIds:["inWareListSaveBtn"]};_permissionList=checkPermissionUi(_permissionList);$(".ms2").select2();$("#cgTime").val(getDate()).datepicker("update");new Promise(n=>getValuer(n)).then(()=>getGroup());$("#groupSelect").on("select2:select",()=>getProcessor());$("#qgProcessor,#formState,#cgProcessor").on("select2:select",()=>getPurchase());$("#inWareList,#purchaseList").on("focus",".zeroNum",function(){$(this).val()==0&&$(this).val("")});$("#inWareList,#purchaseList").on("blur",".zeroNum",function(){isStrEmptyOrUndefined($(this).val())&&$(this).val(0)});$("#purchaseList").on("focus",".numZero",function(){$(this).text().trim()==0&&$(this).text("")});$("#purchaseList").on("blur",".numZero",function(){isStrEmptyOrUndefined($(this).text().trim())&&$(this).text(0)});$("#purchaseList").on("input",".taxTate",function(){onInput(this,2,2);tFootTrCount.call(this)});$("#purchaseList").on("input",".numZero",function(){tFootTrCount.call(this)});$("#departmentAll").on("ifChanged",function(){$(this).is(":checked")?$("#departmentItem .icb_minimal").iCheck("check"):_departmentItem.length==$("#departmentItem").children().length&&$("#departmentItem .icb_minimal").iCheck("uncheck")});$("#departmentItem").on("ifChanged",".icb_minimal",function(){const n=$(this).parents(".departmentLi")[0];$(this).is(":checked")?(_departmentItem.push(n),_departmentItem.length==$("#departmentItem").children().length&&$("#departmentAll").iCheck("check")):(_departmentItem.splice(_departmentItem.indexOf(n),1),_departmentItem.length==$("#departmentItem").children().length-1&&$("#departmentAll").iCheck("uncheck"))})}function tFootTrCount(){const n=$(this).parents("tr"),i=parseFloat(n.find(".number").text())||0,r=parseFloat(n.find(".price").text())||0,u=parseFloat(n.find(".taxTate").val())||0,t=parseFloat((r*(1+u/100)).toFixed(5)),f=parseFloat((i*t).toFixed(5));n.find(".tax-price").text(t);n.find(".tax-amount").text(f);tFootCount()}function tFootCount(){const n=_purchaseDataTable.columns(8).nodes()[0].map(n=>$(n).text()),i=_purchaseDataTable.columns(9).nodes()[0].map(n=>$(n).text()),r=_purchaseDataTable.columns(11).nodes()[0].map(n=>$(n).text()),t=[];for(let u=0,f=n.length;u<f;u++)t[u]={Number:n[u],Price:i[u],TaxAmount:r[u]};computePrice(t)}function pasteTable(n){if(n.originalEvent.clipboardData&&n.originalEvent.clipboardData.items){if(n.target.localName!="input"&&$(n.target).prop("contenteditable")!="true"){const r=(n.originalEvent.clipboardData||window.clipboardData).getData("text"),t=r.split("\n"),i=[];if(_pasteData)for(let n=0,r=_pasteData.length;n<r;n++){const r=_pasteData[n];t[n]?r.push(...t[n].split("\t")):r.push(...new Array(t[0].split("\t").length).fill(""));i.push({Id:0,ItemId:0,Order:r[0]||"",Code:r[1]||"",Name:r[2]||"",Specification:r[3]||"",Class:r[4]||"",Supplier:r[5]||"",Unit:r[6]||"",Number:r[7]||"",Price:r[8]||"",TaxPrice:r[9]||"",TaxAmount:r[10]||"",TaxTate:r[11]||0})}else{_pasteData=[];for(let n=0,r=t.length;n<r;n++){let r=t[n];isStrEmptyOrUndefined(r)||(r=r.split("\t"),_pasteData[n]=r,i.push({Id:0,ItemId:0,Order:r[0]||"",Code:r[1]||"",Name:r[2]||"",Specification:r[3]||"",Class:r[4]||"",Supplier:r[5]||"",Unit:r[6]||"",Number:r[7]||"",Price:r[8]||"",TaxPrice:r[9]||"",TaxAmount:r[10]||"",TaxTate:r[11]||0}))}}setPurchaseList(i);for(let n=0,t=_pasteData.length;n<t;n++)if(_pasteData[n].length>8){_pasteData=null;break}}n.stopPropagation();n.preventDefault()}}function getGroup(n){_departmentItem=[];const t={};t.opType=871;t.opData=JSON.stringify({menu:!0});ajaxPost("/Relay/Post",t,t=>{if(t.errno!=0){layer.msg(t.errmsg);return}const i=t.datas,f=`<div class="flexStyle departmentLi">
                        ...
                        <input class="form-control hidden departmentName" maxlength="20" style="flex-basis:150px;margin-left:5px">
                           ...
                           </tfoot>`;this.find("tfoot").remove();this.append(i);computePrice(n);this.find("tfoot tr:last th").css("borderTop",0)}})}function computePrice(n){let t=0,i=0;for(let r=0,u=n.length;r<u;r++){const u=n[r],f=parseFloat(u.TaxAmount);t=floatObj.add(t,f);i=floatObj.add(i,floatObj.subtract(f,floatObj.multiply(parseFloat(u.Number),parseFloat(u.Price))))}$("#purchaseAmount").text(t?parseFloat(t.toFixed(5)):0);$("#purchaseTax").text(i?parseFloat(i.toFixed(5)):0)}function addPurchaseTr(){_purchaseDataTable.row.add({Id:0,ItemId:0,Order:"",Code:"",Name:"",Specification:"",Class:"",Supplier:"",Unit:"",Number:0,Price:0,TaxPrice:0,TaxAmount:0,TaxTate:0}).draw(!1)}function delPurchaseTr(){const n=$(this).parents("tr")[0],t=$(n).find("td:first").text()>>0;_purchaseDataTable.row(n).remove().draw(!1);_purchaseDataTable.column(0).nodes().each(n=>{const i=$(n).text()>>0;i>t&&$(n).text(i-1)});tFootCount()}function resetPurchaseList(){$("#purchasingCompany").val("");$("#addPurchaseListBtn").addClass("hidden");_purchaseDataTable&&(_pasteData=null,_purchaseDataTable.destroy(),_purchaseDataTable.clear(),_purchaseDataTable=null,$("#purchaseList").empty())}function printPurchaseList(){if(!_purchaseDataTable||!_purchaseDataTable.data().length){layer.msg("入库单为空");return}const t=$("#purchasingCompany").val().trim();if(isStrEmptyOrUndefined(t)){layer.msg("采购公司不能为空");return}let n="";_purchaseDataTable.column(1).nodes().each(t=>{const i=$(t).text().trim();n.includes(i)||(n+=`<option value=${i}>${i}</option>`)});const i=`<label class="control-label text-nowrap">订单号选择：</label><select class="form-control" id="purchaseCode">${n}</select>`;layer.confirm(i,{btn:["打印","取消"],success:function(){this.enterEsc=function(n){return n.keyCode===13?($(".layui-layer-btn0").click(),!1):n.keyCode==27?($(".layui-layer-btn1").click(),!1):void 0};$(document).on("keydown",this.enterEsc)},end:function(){$(document).off("keydown",this.enterEsc)}},function(n){const f=$("#purchaseCode").val();if(isStrEmptyOrUndefined(f)){layer.msg("订单号不能为空");return}layer.close(n);let r="";const e=_purchaseDataTable.rows().nodes(),o=[2,3,5,4,7,8,9];let s=0,i=0;for(let n=0,t=e.length;n<t;n++){const u=$(e[n]),t=u.find("td");if(t.eq(1).text().trim()==f){s++;let n=`<td>${s}</td>`;for(let i=0,r=o.length;i<r;i++)n+=t.eq(o[i]).prop("outerHTML");const f=floatObj.multiply(parseFloat(t.eq(8).text().trim()),parseFloat(t.eq(9).text().trim())).toFixed(2);n+=`<td>${f}</td>`;const u=parseFloat(t.eq(11).text().trim());i=floatObj.add(i,u);n+=`<td>${floatObj.subtract(u,f).toFixed(2)}</td>`;n+=`<td>${u.toFixed(2)}</td>`;n+=`<td>${_Purchase.name}</td>`;r+=`<tr style="height:40px">${n}</tr>`}}r+='<tr style="height:40px"><td>备注：<\/td><td colspan="11"><\/td><\/tr>';i=i.toFixed(2);r+=`<tr style="height:40px"><td colspan="8">合计金额：${convertCurrency(i)}</td><td colspan="4">￥${i}</td></tr>`;const h=`<table border="1" style="width:100%;text-align:center;border-collapse:collapse">${"<thead><tr><th>编号<\/th><th>物料编码<\/th><th>名称<\/th><th>类别<\/th><th>规格<\/th><th>单位<\/th><th>入库数量<\/th><th>单价<\/th><th>不含税金额<\/th><th>税额<\/th><th>合计金额<\/th><th>请购人<\/th><\/tr><\/thead>"}<tbody>${r}</tbody></table>`,c=$(_purchaseDataTable.column(6).nodes()[0]).text();let u=$("#cgTime").val().trim();const l=["年","月","日"];u&&(u=u.split("-").map((n,t)=>`${n}${l[t]}`).join(""));const a=`<div style="text-align:center">${t}</div><h3 style="text-align:center;margin:5px;letter-spacing:8px">采购入库单</h3>`,v=`<div style="text-align:right">订单号：${f}</div>`,y=`<div style="position:absolute;right:0">供应商：${c}</div><div style="text-align:center;width:100%">日期：${u}</div>`,p=`<div style="display:flex;margin:8px 0"><label style="flex:1">主管：</label><label style="flex:1">仓库：<span style="margin-left:20px">${getCookieTokenInfo().name}</span></label><label style="flex:1">记账：</label><label style="flex:1">采购人：<span style="margin-left:20px">${_purchaseDataTable.data()[0].Purchaser||""}</span></label></div>`,w=`<div style="position:relative">${a}${v}${y}${h}${p}</div>`;printCode(w)},function(n){layer.close(n)})}var _permissionList=[],_pasteData=null;let _departmentItem=null;let _stateDataTable=null,_Purchase=null,_inWareDataTable=null;let _trChangeData=null;let _purchaseDataTable=null;