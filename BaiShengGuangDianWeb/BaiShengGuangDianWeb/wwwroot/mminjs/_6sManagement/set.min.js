function pageReady(){_permissionList[634]={uIds:["saveItemListBtn"]};_permissionList[635]={uIds:["delGroupBtn"]};_permissionList=checkPermissionUi(_permissionList);$(".ms2").select2();$(".ms3").select2({allowClear:!0,placeholder:"请选择",multiple:!0});$("#6sGroupChangeCheck").iCheck("uncheck");$("#6sGroupChangeCheck").on("ifChanged",function(){$(this).is(":checked")?$("#6sGroupChangeDiv").removeClass("hidden"):$("#6sGroupChangeDiv").addClass("hidden")});$("#6sGroupSelect").on("select2:select",function(){initSurveyorSelect();getItemList()});init()}function init(n=0){var t=new Promise(function(t){getGroupList(t,n)}),i=new Promise(function(n){getSurveyorList(n)});Promise.all([t,i]).then(()=>{initSurveyorSelect(),getItemList()})}function _6sGroupChangeLabel(){var n=$("#new6sGroupTxt").val();isStrEmptyOrUndefined(n)&&$("#6sGroupChangeCheck").iCheck($("#6sGroupChangeCheck").is(":checked")?"uncheck":"check")}function new6sGroupTxt(){var n=$("#new6sGroupTxt").val();isStrEmptyOrUndefined(n)?($("#6sGroupSelect").removeAttr("disabled"),$("#6sGroupChangeCheck").removeAttr("disabled"),$("#6sGroupChangeCheckLbl").removeAttr("disabled")):($("#6sGroupChangeCheck").iCheck("uncheck"),$("#6sGroupChangeDiv").addClass("hidden"),$("#6sGroupSelect").attr("disabled","disabled"),$("#6sGroupChangeCheck").attr("disabled","disabled"),$("#6sGroupChangeCheckLbl").attr("disabled","disabled"))}function getGroupList(n,t=0){var i={};i.opType=900;ajaxPost("/Relay/Post",i,function(i){var r,f;if(n!=null&&n("success"),i.errno!=0){layer.msg(i.errmsg);return}$("#6sGroupSelect").empty();r=i.datas;_groups=r;for(var o=r.length,e="",u=0;u<o;u++)f=r[u],e+='<option value = "{0}">{1}<\/option>'.format(f.Id,f.Group);$("#6sGroupSelect").append(e);$("#6sGroupSelect").val(t).trigger("change")})}function getSurveyorList(n){var t={};t.opType=254;ajaxPost("/Relay/Post",t,function(t){var i,f;if(n!=null&&n("success"),t.errno!=0){layer.msg(t.errmsg);return}$("#surveyorSelect").empty();i=t.datas;_surveyors=i;for(var e=i.length,u="",r=0;r<e;r++)f=i[r],u+='<option value = "{0}">{1}<\/option>'.format(f.Id,f.SurveyorName);_surveyorOptions=u;$("#surveyorSelect").append(u)})}function initSurveyorSelect(){var i,n,t;if($("#surveyorSelect").val("").trigger("change"),i=$("#6sGroupSelect").val(),!isStrEmptyOrUndefined(i)&&_groups&&_groups.length>0)for(n=0;n<_groups.length;n++)t=_groups[n],t.Id==i&&($("#surveyorSelect").attr("old",t.SurveyorId),$("#surveyorSelect").val(t.SurveyorId.split(",")).trigger("change"))}function getItemList(){var t,n;$("#itemList").empty();t=$("#6sGroupSelect").val();isStrEmptyOrUndefined(t)||(n={},n.opType=904,n.opData=JSON.stringify({qId:t}),ajaxPost("/Relay/Post",n,function(n){if(n.errno!=0){layer.msg(n.errmsg);return}item=n.datas;initItemList()}))}function initItemList(){var r,i;if($("#6sGroupChangeCheck").iCheck("uncheck"),$("#6sGroupChangeDiv").addClass("hidden"),$("#6sGroupChangeTxt").val(""),$("#itemList").empty(),itemMax=itemMaxV=0,item&&item.length>0){for(itemMax=itemMaxV=item.length,r="",i=0;i<item.length;i++){var t=i+1,n=item[i],u=`<tr id="item${t}" value="${t}" xh="${t}" itemId="${n.Id}">
            ...
            </tr>`;$("#itemList").append(i);t="#item"+n;$(t).find(".ms2").select2({width:"120px"});$(t).find(".enable").iCheck({handle:"checkbox",checkboxClass:"icheckbox_polaris",increaseArea:"20%"}).iCheck("check")}function delItemList(n){var i,r,t,u;for($("#itemList").find(`tr[value=${n}]:first`).remove(),itemMaxV--,i=1,r=$("#itemList tr"),t=0;t<r.length;t++)$(r[t]).attr("xh",i),u=$(r[t]).attr("value"),$("#itemXh"+u).html(i),i++}function saveItemList(){var a=$("#new6sGroupTxt").val(),k=$("#6sGroupChangeTxt").val(),d=$("#surveyorSelect").val(),t={},u=0,f=0,i=!1,g,v,y,s,h,p,c,w,l,e,o,b,r,rt;if(isStrEmptyOrUndefined(a)){if(f=901,u=$("#6sGroupSelect").val(),isStrEmptyOrUndefined(u)){layer.msg("请选择分组");return}t.Id=u;$("#6sGroupChangeCheck").is(":checked")&&$("#6sGroupSelect option:selected").text()!=k&&(i=!0,t.Group=k)}else{if(isStrEmptyOrUndefined(a))return;f=902;i=!0;t.Group=a}g=isStrEmptyOrUndefined($("#surveyorSelect").attr("old"))?"":$("#surveyorSelect").attr("old");v=d==null?"":d.join();f==901&&g!=v&&(i=!0);t.SurveyorId=v;for(var nt=[],tt=0,it=0,n=1;n<=itemMax;n++)if($("#item"+n).length>0){if(y=$("#item"+n).attr("itemId"),r=0,y==0?(r=2,it++):(tt++,$("#itemChose"+n).is(":checked")?(r=2,i=!0):r=1),r==1)s=$("#itemOrder1"+n).html(),h=$("#itemName1"+n).html(),p=$("#itemEnable"+n).is(":checked"),c=$("#itemStandard1"+n).html(),w=$("#itemReference1"+n).attr("title"),l=$("#itemInterval1"+n).attr("interval"),e=$("#itemInterval1"+n).attr("day"),o=$("#itemInterval1"+n).attr("week"),b=$("#itemPerson1"+n).attr("old");else if(r==2)s=$("#itemOrder2"+n).val(),h=$("#itemName2"+n).val(),p=$("#itemEnable"+n).is(":checked"),c=$("#itemStandard2"+n).val().trim(),w=$("#itemReference2"+n).val().trim(),l=$("#itemInterval22"+n).val().trim(),l==1?(e=$("#itemInterval21"+n).val().trim(),o=0):(e=0,o=$("#itemInterval21"+n).val().trim()),b=$("#itemPerson2"+n).val();else return;if(isStrEmptyOrUndefined(s)){layer.msg("请输入顺序");return}if(isStrEmptyOrUndefined(h)){layer.msg("请输入检查项目");return}if(isStrEmptyOrUndefined(c)){layer.msg("请输入标准分");return}if(e+o<=0){layer.msg("请输入频次");return}nt.push({Id:y,Order:s,Item:h,GroupId:u,Enable:p,Standard:c,Reference:w,Interval:l,Day:e,Week:o,Person:b})}(t.Items=nt,(it>0||f==901&&tt!=item.length)&&(i=!0),i)&&(rt=function(){var n={};n.opType=f;n.opData=JSON.stringify(t);ajaxPost("/Relay/Post",n,function(n){layer.msg(n.errmsg);n.errno==0&&init(u)})},showConfirm("保存",rt))}function delGroup(){var n=$("#6sGroupSelect").val(),t;if(isStrEmptyOrUndefined(n)){layer.msg("请选择分组");return}t=function(){var t={};t.opType=903;t.opData=JSON.stringify({ids:[n]});ajaxPost("/Relay/Post",t,function(n){layer.msg(n.errmsg);n.errno==0&&init()})};showConfirm("删除分组："+$("#6sGroupSelect option:selected").text(),t)}var _permissionList=[],_groups=null,_surveyors=null,_surveyorOptions="",itemMax=0,itemMaxV=0,item=null,itemLength=0;